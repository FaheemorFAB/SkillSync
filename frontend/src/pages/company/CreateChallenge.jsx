  ...
  return (
    <div>
      <div className="page-header">
        ...
        <p className="text-slate-500 text-sm mt-1">Use AI to auto-generate test content from your problem description</p>
      </div>

      <form onSubmit={handleSubmit} className="page-body max-w-3xl space-y-8">
        {/* AI Generator */}
        <div className="p-5 rounded-2xl" style={{ background: 'linear-gradient(135deg, #EEF2FF, #F5F3FF)', border: '1px solid #C7D2FE' }}>
          <div className="flex items-center gap-2 mb-3">
            <Sparkles size={16} style={{ color: '#6366F1' }} />
            <span className="font-semibold text-indigo-700 text-sm">AI Problem Creator</span>
            <span className="badge badge-live text-xs">Powered by Gemini 2.5</span>
          </div>
          <p className="text-xs text-slate-500 mb-3">Describe your business problem and let AI generate test questions</p>
          <textarea
        ...
        <section className="space-y-5">
          <h2 className="font-bold text-slate-900">Challenge Details</h2>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {SECTORS.map(s => (
                <button key={s.id} type="button" onClick={() => setForm(f => ({ ...f, sector: s.id, sub_sector: '' }))}
                  ...
                  style={{ borderColor: form.sector === s.id ? s.color : '#E2E8F0', background: form.sector === s.id ? `${s.color}15` : '' }}>
              ...
              <div className="flex gap-2 flex-wrap">
                {selectedSector.subs.map(s => (
                  <button key={s} type="button" onClick={() => setForm(f => ({ ...f, sub_sector: s }))}
                    ...
                    style={{ borderColor: form.sub_sector === s ? selectedSector.color : '#E2E8F0', background: form.sub_sector === s ? `${selectedSector.color}15` : 'transparent', color: form.sub_sector === s ? selectedSector.color : '#64748B' }}>
                    {s}
                  </button>
                ))}
              </div>
        ...
        {/* Round content */}
        <section className="space-y-5">
          <h2 className="font-bold text-slate-900">Round Content (AI-filled or manual)</h2>
          <p className="text-xs text-slate-500">Round 1 (Aptitude MCQs) are auto-generated by AI when a candidate starts. Configure rounds 2 &amp; 3 below.</p>
